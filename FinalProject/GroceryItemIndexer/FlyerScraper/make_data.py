...
for file in ['train','test']:
    ...
    with open(labels_txt, "w") as wf:
        ...
        while image_num < images_num:
            ...
            text_path = os.path.realpath(os.path.join(images_path, "%06d.txt" %(image_num+1)))
            #print(image_path)
            ...
            with open(text_path,"w") as separateFile:
                print(image_path)
                ...
                for i in range(len(labels)):
                    ...
                    x_pos = x1 / SIZE + ((x2 - x1) / SIZE /  2)
                    y_pos = y1 / SIZE + ((y2 - y1) / SIZE / 2)
                    x_size = (x2 - x1) / SIZE
                    y_size = (y2 - y1) / SIZE
                    #heigh and width is 416
                    
                    results.append([oclass,x_pos,y_pos,x_size,y_size])
                    
                    annotation +=  ' '.join([str(oclass), str(x_pos), str(y_pos), str(x_size), str(y_size)])+ '\n' 
                image_num += 1
                print("=> %s" %annotation)
                wf.write(annotation )
                separateFile.write(annotation)
